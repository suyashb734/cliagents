...
async function run() {
  ...
  try {
    ...
    assert(routePage.text.includes('/orchestration/root-sessions?limit=80&scope='), '/console should use scoped root-session summaries for the session tree list');
    ...
    assert(routePage.text.includes("bindTerminalDetailActions(rootTerminalBundle.terminal, 'root-terminal'"), '/console should wire reply and control actions for the root terminal');
    assert(routePage.text.includes('/orchestration/terminals/${encodeURIComponent(rootTerminalId)}/output?lines=250'), '/console should load root terminal output inside the root-session detail view');
