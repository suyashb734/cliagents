...
- **Memory snapshot**: derived continuity bundle over root, run, room, task, or
  project history. Task bundles include runs, assignments, linked rooms, usage,
  and recent root session events; project bundles roll up recent tasks, runs,
  rooms, and project usage.
